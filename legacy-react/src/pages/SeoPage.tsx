...
import { BRAND, SERVICES, MODELS, CITIES, STEPS, PHONE_HREF, generateSeoPages } from "@/lib/data";
...
export default function SeoPage() {
  ...
  const faq = [
    { q: `Колко струва ${service.name.toLowerCase()} на ${model.name}?`, a: `Цената за ${service.name.toLowerCase()} на ${model.name} зависи от диагностиката. Цените започват от ${service.priceFrom} €. Диагностиката е безплатна.` },
  ...
  return (
    <Layout>
      <SEOHead
        title={`${service.name} ${model.name} – от ${service.priceFrom} € | ${BRAND}`}
            ...
            {SERVICES.filter((s) => s.slug !== service.slug).map((s) => {
              ...
              return (
                <Link key={s.slug} to={`/${seoSlug}`} className="card-service text-center group">
                  ...
                  <p className="text-primary font-bold mt-2">от {s.priceFrom} €</p>
                </Link>
              );
            })}
